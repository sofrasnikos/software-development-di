...
NgramCounter *create_ngram_counter () {
    ...
    for (int i = 0; i < NC_STATIC_HASH_SIZE; i++) {
        allocateNCBucketArray(&(ngramCounter->buckets[i]));
    }
    return ngramCounter;
};
...
void destroy_gram_counter(NgramCounter *ngramCounter) {
    for (int i = 0; i < NC_STATIC_HASH_SIZE; i++) {
        destroyNCbucketArray(&(ngramCounter->buckets[i]));
    }
    free(ngramCounter);
};
...
void print_ngram_counter(NgramCounter *ngramCounter) {
    for (int i = 0; i < NC_STATIC_HASH_SIZE; i++) {
        printf("Bucket[%d]", i);
        printNCBucketArray(&(ngramCounter->buckets[i]));
    }
}